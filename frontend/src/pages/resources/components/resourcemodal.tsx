...
import { X, BookOpen, Link2, FileText, Tag, Star } from "lucide-react";
import { useSubjects } from "../../../context/subjectcontext";
import { useGoals } from "../../../context/goalcontext";
import { useTasks } from "../../../context/taskcontext";
import type { Resource, ResourceType, CreateResourceData } from "../../../context/resourcecontext";

interface ResourceModalProps {
  open: boolean;
  resource: Resource | null;
  onClose: () => void;
  onSave: (data: CreateResourceData) => void;
}
...
export default function ResourceModal({
  ...
  useEffect(() => {
    ...
    if (resource) {
      ...
      setGoalId(resource.goalId || "");
      setTaskId(resource.taskId || "");
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-[#1F2937] bg-[#0B1120] p-6 shadow-2xl space-y-4 scrollbar-none"
        ...
        <div className="flex items-center justify-between border-b border-[#1F2937] pb-3">
          <div>
            ...
            <p className="text-xs text-[#64748B]">
              Organize external links, documentation, videos, and articles.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. OSI Layer & Protocol Guide"
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#CBD5E1] flex items-center gap-1">
              <Link2 size={12} className="text-[#64748B]" /> Resource Link (URL)
            </label>
            <input
              type="url"
              value={url}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-[#CBD5E1] flex items-center gap-1">
              <FileText size={12} className="text-[#64748B]" /> Description
            </label>
            <textarea
              rows={3}
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Key contents or summary of this resource..."
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1">
              ...
              <input
                ...
                placeholder="e.g. Networking, OSI, Guide"
                className="w-full rounded-lg border border-[#1F2937] bg-[#020617] p-2.5 text-xs text-[#E5E7EB] outline-none"
              />
            </div>
